...
#==============================================================================#
"""USAGE
Module imported and used for both the "3a_Temporal Persistence Analysis (day to day)"
and "3b_Temporal Persistence Analysis (year to year)" tool scripts in the
"GEM2_Oil_Seep_Detection_Analysis" Python Toolbox.

SUMMARY
Performs a spatial-temporal analysis of the persistence of dark targets on either
a day to day basis within the year or an overall year to year basis.

INPUT
- Year Dataset or Location of GEM2 Temporal Analysis GDB (user input): Location
of input feature classes depending on type of analysis (day to day or year to year)
...
A shapefile or feature class of dark targets produced outside of this toolbox to
 be incorporated as additional data in a multi-year persistence analysis. The ID
  ...
  which indicates the year of the dark target. This is only used for a year to year
  analysis.

OUTPUT
- Persistence Field (automated output): Attribute field created as 'pers*' for a
day-to-day analysis within the year and 'Ypers*' for a year-to-year overall analysis.
(e.g. pers8000 for a day-to-day persistence value with a buffer of 8,000 meters,
 or Ypers10000 for a year-to-year persistence value with a buffer of 10,000 meters)

    Value is the number of links from one dark target to another from a different time frame (For
     example, a dark target from 2011 which has a dark target from 2012 within its
     buffer distance will have a persistence value of 1. If a dark target from 2010
     was also within its buffer distance, the dark target from 2011 would have a
     persistence value of 2)

- Weight Field (automated output): Attribute field is created as 'wght*' for a
day-to-day analysis within the year and 'Ywght*' for a year-to-year overall analysis.
(e.g. pers8000 for a day-to-day persistence value with a buffer of 8,000 meters,
 or Ypers10000 for a year-to-year persistence value with a buffer of 10,000 meters)

    Value is the number of dark targets considered persistent within the buffer
...
- Cluster ID Field (automated output): Attribute field is created as 'clst*' for
a day-to-day analysis within the year and 'Yclst*' for a year-to-year overall
analysis. (e.g. clst8000 for a day-to-day cluster ID value for a buffer of 8,000
meters, or Yclst10000 for a year-to-year cluster ID value for a buffer of 10,000
meters)

    Value is formed by a set of two numbers separated by a decimal point. The first
    number, before the decimal point, is an arbitrary identifying number assigned
    during the clustering process. The second number, after the decimal point,
    indicates the time span in months between the oldest dark target in the cluster
    and the most recent dark target in the cluster. (e.g., a cluster with the
    ID 17.24 was the 14th cluster to be processed, and has a time span of 24 months
    between the oldest and most recent members of the cluster.

- Consolidated feature class (automated output): Final output feature class which
consolidates all the input dark targets together in a single output feature class,
with their associated persistence values.
...
class temporalPersistence(object):
    ...
    def calcPersis(self, workspace, yrPersisBool, targetIDfeatList, pointList, bufferDist):
        ...
        Return:
            ...
            Weight = Number of dark targets considered persistent within the buffer
            distance of the dark target. (For example, a dark target from 2011 which
            has two dark targets from 2010 within its buffer distance is considered
            persistent (at a value of 1), and has a weight value of 3 (1 for itself,
            and 1 for each dark target within its buffer distance)"""
